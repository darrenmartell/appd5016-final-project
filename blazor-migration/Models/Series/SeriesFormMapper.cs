...
public static class SeriesFormMapper
{
    ...
    public static SeriesUpsertRequest ToRequest(SeriesFormInput input)
    {
        return new SeriesUpsertRequest
        {
            ...
            RuntimeMinutes = ParseNumberOrZero(input.RuntimeMinutes),
            ReleasedYear = ParseNumberOrZero(input.ReleasedYear),
            ...
            {
                Imdb = ParseNullableDouble(input.Ratings.Imdb),
                RottenTomatoes = ParseNullableDouble(input.Ratings.RottenTomatoes),
                Metacritic = ParseNullableDouble(input.Ratings.Metacritic),
                UserAverage = ParseNullableDouble(input.Ratings.UserAverage)
            },
