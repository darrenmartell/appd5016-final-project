...
| BLZ-008 | The users placeholder was replaced in Phase 3, and the series slice was completed in Phase 4. | 1 | Low | Migration agent | 4 | Closed |
...
| BLZ-011 | Live Phase 4 validation showed that the deployed series write endpoints reject React-style blank-to-`0` values for `runtime_minutes` and `released_year`, and they also reject missing or null rating fields on write. The Blazor form now validates those fields as required, but this remains a documented backend contract mismatch against the original React submit transform. | 4 | Medium | Migration agent | 5 | Open |